...
private struct One15STS: Decodable { let AccessKeyId:String?; let AccessKeyID:String?; let AccessKeySecret:String; let SecurityToken:String; var key:String { AccessKeyId ?? AccessKeyID ?? "" } }
private final class One15Client {
 ...
 func ossToken() async throws -> One15STS { try JSONDecoder().decode(One15STS.self,from:data(req(URL(string:"https://uplb.115.com/3.0/gettoken.php")!))) }
 func initialize(file:URL,name:String,cid:String,size:Int64,fileid:String,preid:String,userID:Int64,userkey:String) async throws -> One15Init {
  ...
  while true { let t=Int64(Date().timeIntervalSince1970*1000);let token=Digest115.md5("Qclm8MGWUv59TnrR0XPg\(fileid)\(size)\(sk)\(sv)\(userID)\(t)\(Digest115.md5(String(userID)))27.0.5.7");var f=["appid":"0","appversion":"27.0.5.7","userid":"\(userID)","filename":name,"filesize":"\(size)","fileid":fileid,"target":target,"sig":signature,"topupload":"true","t":"\(t)","token":token];if !sk.isEmpty {f["sign_key"]=sk;f["sign_val"]=sv};let body=try crypto.encryptRequest(Form115.encode(f).data(using:.utf8)!);var c=URLComponents(string:"https://uplb.115.com/4.0/initupload.php")!;c.queryItems=[URLQueryItem(name:"k_ec",value:try crypto.token(forMilliseconds:t))];var r=req(c.url!,"POST",body);r.setValue("application/x-www-form-urlencoded",forHTTPHeaderField:"Content-Type");let result=try JSONDecoder().decode(One15Init.self,from:try crypto.decryptResponse(data(r)));if result.status==7,let key=result.sign_key,let range=result.sign_check {let a=range.split(separator:"-").compactMap{UInt64($0)};guard a.count==2 else{throw One15Error.message("115 校验范围错误")};sk=key;sv=try Digest115.sha1(file,offset:a[0],count:a[1]-a[0]+1);continue};guard result.status==1 || result.status==2 else{throw One15Error.message(result.statusmsg ?? "115 初始化上传失败")};return result }
 }
}
